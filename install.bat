...
:: latest versions
set "latest_OCA=3.0.5"
set "program_version=0.0.12"
...
if "%output%"=="4" (call :DoInstall spotify)
if "%output%"=="0" (goto :menu) else (
   echo Please enter a number!
   pause
...
if "%dotnet10%"=="1" (
   ...
   if "%os_bit%"=="x64" (
      echo .NET Framework 1.0 doesn't work on Windows XP x64 editions. Skipping...
   ) else (
...
if "%selected%"=="1" (
   ...
   echo Downloading archive, please wait...
    "%CURL_PATH%" -# -L -o spotify.zip https://github.com/Snaky1a/xp-apps/releases/download/2023_10_11_17_52/Spotify.zip

    echo Extracting, please wait...
    "%ZIP_PATH%" x spotify.zip -y -bsp2 -bso0

    echo Path to Spotify: "%current_path%\Spotify\".
    echo You may run spotify from batch script (%current_path%\Spotify\start.bat)
    pause
    goto :menu
) else (
   goto :eof
